...
{
    /// <summary>
    /// Class for generating the mesh of the map 
    /// </summary>
    public static class MeshGenerator
    {
        public static MeshData GenerateTerrainMesh(float[,] heightMap, MeshSettings meshSettings, int levelOfDetails)
        {
            int skipIncrement = (levelOfDetails == 0) ? 1 : levelOfDetails * 2;
            ...
            for (int y = 0; y < numVertsPerLine; y++)
            {
                for (int x = 0; x < numVertsPerLine; x++)
                {
                    ...
                    if (isEdgeConnectionVertex)
                    {
                        ...
                        int distFromMainVertexA = ((isVertical) ? (y - 2) : (x - 2)) % skipIncrement;
                        int distFromMainVertexB = skipIncrement - distFromMainVertexA;

                        float distPercentFromAToB = distFromMainVertexA / (float)skipIncrement;

                        float heightMainVertexA = heightMap[(isVertical) ? x : x - distFromMainVertexA,
                            (isVertical) ? y - distFromMainVertexA : y];
                        float heightMainVertexB = heightMap[(isVertical) ? x : x + distFromMainVertexB,
                            (isVertical) ? y + distFromMainVertexB : y];

                        height = heightMainVertexA * (1 - distPercentFromAToB) +
                                 heightMainVertexB * distPercentFromAToB;
                    }
    ...
    /// <summary>
    ///  MeshData class is used to store the vertices, triangles and uvs of the mesh
    /// </summary>
